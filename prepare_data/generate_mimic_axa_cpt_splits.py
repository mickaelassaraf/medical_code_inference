...
import pandas as pd
from src.settings import (DATA_DIRECTORY_MIMIC_AXA_CPT, ID_COLUMN,
...
output_dir_cpt = Path(DATA_DIRECTORY_MIMIC_AXA_CPT)

axa_cpt = pd.read_feather(output_dir_cpt / "mimic_axa_cpt.feather")
axa_cpt[TARGET_COLUMN] = axa_cpt[TARGET_COLUMN].apply(lambda x: list(x))
...
generate_split(axa_cpt, output_dir_cpt / "mimic_axa_cpt_split.feather")
