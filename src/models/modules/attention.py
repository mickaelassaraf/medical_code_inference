...
from src.utils.preprocess_permutation import (
    create_permutations_cpt,
    create_permutations_icd9,
    create_permutations_icd10,
)
...
class LabelAttentionHierarchicalCPT(nn.Module):
    def __init__(self, input_size: int, projection_size: int, num_classes: int, label_transform):
        ...
        self.embedding_one = nn.Linear(projection_size, self.permutation_matrix_1.shape[0])
        self.third_linear = nn.Linear(input_size, num_classes)
...
class LabelAttentionHierarchicalICD9(nn.Module):
    def __init__(self, input_size: int, projection_size: int, num_classes: int,label_transform):
        super().__init__()
        self.first_linear = nn.Linear(input_size, projection_size, bias=False)
        
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.permutation_matrix_chapter,self.permutation_matrix_section,self.permutation_matrix_level_1,self.permutation_matrix_all = create_permutations_icd9(label_transform, self.device)
        
        self.full_embedding = nn.Linear(projection_size, num_classes)
        self.embedding_third = nn.Linear(projection_size, self.permutation_matrix_level_1.shape[0])
        self.embedding_second = nn.Linear(projection_size, self.permutation_matrix_section.shape[0])
        self.embedding_one =nn.Linear(projection_size, self.permutation_matrix_chapter.shape[0])
        self.third_linear = nn.Linear(input_size, num_classes)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Combine the embeddings with permutation matrices here
        att_weights = self.full_embedding(weights)@ self.permutation_matrix_all
        att_weights += self.embedding_third(weights) @self.permutation_matrix_level_1
        att_weights +=  self.embedding_second(weights)@self.permutation_matrix_section 
        att_weights += self.embedding_one(weights) @ self.permutation_matrix_chapter
...
class LabelAttentionHierarchicalICD10(nn.Module):
    def __init__(self, input_size: int, projection_size: int, num_classes: int,label_transform):
        super().__init__()
        self.first_linear = nn.Linear(input_size, projection_size, bias=False)
        self.full_embedding = nn.Linear(projection_size, num_classes)
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.permutation_matrix_1,self.permutation_matrix_2,self.permutation_matrix_3,self.permutation_matrix_all= create_permutations_icd10(label_transform, self.device)
